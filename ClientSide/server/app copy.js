...
const FuctionForTrade=(order, nameofAccount)=>{
  ...
  var ordr = mrkt.replace("<INS>","NQ ${digits_rep_date}").replace("<ACT>",action).replace("<QTY>",amount);
  if( Currentvalues['action'].includes("FLAT")){
     ordr = "CLOSEPOSITION;<ACCOUNT>;<INSTRUMENT>;;;;;;;;;;".replace("<ACCOUNT>",nameofAccount).replace("<INSTRUMENT>","NQ ${digits_rep_date}");
  }
