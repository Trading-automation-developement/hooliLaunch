...
socket.on('TradeNow', (data) => {
  console.log('TradeNow', data["d"], data["INS"], new Date());
...
const FuctionForTrade=(order, nameofAccount, INS)=>{
  ...
  var ordr = mrkt.replace("<INS>",INS).replace("<ACT>",action).replace("<QTY>",amount);
  if( Currentvalues['action'].includes("FLAT")){
     ordr = "CLOSEPOSITION;<ACCOUNT>;<INSTRUMENT>;;;;;;;;;;".replace("<ACCOUNT>",nameofAccount).replace("<INSTRUMENT>",INS);
  }
